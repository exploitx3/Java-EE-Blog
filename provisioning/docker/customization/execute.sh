...
data-source add --name=SoftUniDS --driver-name=mysql --jndi-name=java:jboss/datasources/SoftUniDS --connection-url=jdbc:mysql://$MYSQL_URI/sample?useUnicode=true&amp;characterEncoding=UTF-8 --user-name=mysql --password=mysql --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
